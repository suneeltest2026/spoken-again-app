(() => {
  ...
  // ---------- Learning buddy avatar (cartoon-filtered selfie, home screen) ----------
  ...
  function renderAvatarWidget() {
    ...
    if (saved) el('avatar-image').src = saved;
  }
  ...
  function cartoonify(canvas) {
    ...
    for (let y = 1; y < h - 1; y++) {
      for (let x = 1; x < w - 1; x++) {
        ...
        if (Math.sqrt(gx * gx + gy * gy) > EDGE_THRESHOLD) edge[idx] = 1;
      }
    }
    ...
    for (let p = 0, i = 0; i < data.length; i += 4, p++) {
      if (edgeThick[p]) {
        ...
        out.data[i + 2] = Math.round(Math.round(data[i + 2] / step) * step);
      }
      out.data[i + 3] = 255;
    }
